...
def admin_generate_qr(subject_id):
    ...
    # 🔐 FULL TOKEN (QR ke andar jayega)
    full_token = ''.join(random.choices(
        string.ascii_letters + string.digits, k=32
    ))

    # 👁️ DISPLAY TOKEN (admin + students ko)
    display_token = ''.join(random.choices(string.digits, k=4)) + \
    ...
    conn = get_db()
    conn.execute("""
        INSERT INTO qr_tokens (full_token, display_token, subject_id, expires_at)
        VALUES (?, ?, ?, ?)
